...
impl Invaders {
    pub fn new() -> Self {
        ...
        for x in 0..NUM_COLS {
            for y in 0..NUM_ROWS {
                if (x > 1) && (x < NUM_COLS) && (y > 0) && (y < 9) && (x % 2 == 0) && (y % 2 == 0) {
    ...
    pub fn update(&mut self, delta: Duration) -> bool {
        ...
        if self.move_timer.ready {
            ...
            if downwards {
                let new_direction = max(self.move_timer.duration.as_millis() - 250, 250);
                self.move_timer = Timer::from_millis(new_direction as u64);
